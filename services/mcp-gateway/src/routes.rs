...
pub async fn query(Json(mut req): Json<QueryRequest>) -> Result<Json<ReasoningRequest>, (StatusCode, Json<serde_json::Value>)> {
    ...
    if let Err(e) = crate::model::validation::validate_query_request(&mut req) {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(json!({
...
pub async fn reason(Json(mut reason_req): Json<ReasoningRequest>) -> Result<(StatusCode, Json<ReasoningResponse>), (StatusCode, Json<serde_json::Value>)> {
    ...
    // Sanitize and validate
    if let Err(e) = crate::model::validation::validate_reasoning_request(&mut reason_req) {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(json!({
                "error": "Input validation failed",
...
pub async fn reason_local(Json(mut req): Json<ReasoningRequest>) -> Result<(StatusCode, Json<ReasoningResponse>), (StatusCode, Json<serde_json::Value>)> {
    ...
    // Sanitize and validate
    if let Err(e) = crate::model::validation::validate_reasoning_request(&mut req) {
        return Err((
            StatusCode::BAD_REQUEST,
            Json(json!({
                "error": "Input validation failed",
...
pub async fn verify(Json(resp): Json<ReasoningResponse>) -> Result<Json<VerifyResponse>, (StatusCode, Json<serde_json::Value>)> {
    // Validate input
